#!/usr/bin/env python3
"""
apply_safe.py - Aplicador SEGURO de traducciones in situ a ENExtend.bra.

Toma uno o varios JSON {"EVENT\\SCRIPT\\<id>\\main.CL3": [[offset, len_orig, "es"], ...]}
y los aplica sobre ENExtend.bra con TODAS las salvaguardas:

  * Valida cada línea contra el ORIGINAL (ENExtend.bra.orig): el offset debe ser
  ...
  * Reempaqueta a un archivo temporal, verifica CRC de cada entrada modificada,
    y solo entonces reemplaza ENExtend.bra. Si algo falla, no toca el original.
...
def bra_write(path, ver, ents):
    ...
    open(path, "wb").write(bytearray(b"PDA\x00" + struct.pack("<III", ver, H + len(data), len(ents))) + data + index)

def main():
    game = sys.argv[1]; progress_path = sys.argv[2]; jsons = sys.argv[3:]
    bra = os.path.join(game, "ENExtend.bra")
    orig = os.path.join(game, "ENExtend.bra.orig")
    assert os.path.exists(orig), "falta ENExtend.bra.orig (backup)"
